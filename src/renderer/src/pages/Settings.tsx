...
const Settings = () => {
  ...
  const [pathErrorState, setPathErrorState] = useState<string | null>(null);
  const { itemsPath, cookiesStored, updateConfig } = useSettingsContext();

  const handlePathSave = async () => {
    const value_ = inputPathRef.current?.value;

    if (!value_?.trim().length) {
      setPathErrorState('Il campo non può essere vuoto');
      return;
    } else {
      setPathErrorState(null);
    }

    const value = value_;

    try {
      await window.storeItemsPath(value);

      updateConfig();
    } catch (err) {
      console.log(err);
      setPathErrorState('Impossibile salvare il percorso');
    }
  };

  if (itemsPath && inputPathRef.current) {
    inputPathRef.current.value = itemsPath;
  }
                ...
                {pathErrorState && (
                  <div className="mt-7.5 mb-7.5 flex w-full border-l-6 border-[#F87171] bg-[#F87171] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
                    ...
                    <div className="w-full">
                      ...
                      <ul>
                        <li className="leading-relaxed text-[#CD5D5D]">{pathErrorState}</li>
                      </ul>
                    </div>
                  </div>
                )}
